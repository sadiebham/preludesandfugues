...
that may be there andcertainly more than the rhythms of feet and metre which we hear.
...
Music that really is music must attempt
...
nowadays the usual tuning on a piano but at the time was a  major innovation.

Bach's 48~successfully does all of this and the idea has frequently
been copied, for example by Chopin in his \textit{Preludes}. But it
achieves much more than this.  It is a showcase for composition: each
key inviting the composer to write in a different style, always within
the constraints of an introductory prelude followed by a contrapuntal
fugue\=this word literally meaning `flight' and was by Bach's day a
well understood and developed musical form.  Furthermore, composers,
...
E\fl\ major is associated with the `heroic,' as used for example in
Beethoven's \textit{Eroica} symphony or Strauss's tone
poem \textit{Ein Heldenleben}.\footnote{See the appendix for a more
detailed list. It is of course somewhat na\"{\i}ve to associate a simple
list of moods to musical keys: the mood very much depends on the
composer's treatment of the music too, such as whether it is fast or slow or
a dance or a song, etc.}

Whether these moods associated with keys is an innate part of our
psychological make-up or a synthetic product of the way composers
write music is a moot point, but there is no doubt that pieces of
music in different keys sound very different in terms of mood and
emotion, and this can easily be verified by playing the same music
transposed from one key to another and listening to the differences.

The same applies to literature.  If anything can convey emotion,
literature surely can, and there are considerably more traits
...
mood.  To keep things simple, one's writing could focus on a single
subject, though in music as well as literature `modulations' of the
key or mood are frequent and, for example, the much-used
...
With all this in mind, this volume is a small set of preludes and
fugues in literature. Each author has been asked to focus on particular
moods, emotions or traits and to write about a person, object or place that conveys
them.  Modulations (contrasting emotions) were not discouraged
and indeed Bach's 48~contain many modulations too, but the focus
is on one primary subject.   For that subject the authors were asked to write
an introductory `prelude' setting out the main subject in its key,
followed by a `fugue' or story or poem involving it, which might be
in a different literary style.

Of course, to what extent these pieces are musical is
up to the reader to judge, but we have all found the exercise useful
in finding new ways to think about what we write and in expanding and
developing literary style and technique in writing. We sincerely hope
